...
while true; do
	...
	while read -r playing position length name artist title arturl hpos hlen; do
		...
		case $playing in
		⏸️ | Paused) text='<span foreground=\"#E6C384\" size=\"smaller\">'"$line"'</span>' ;;
		▶️ | Playing) text="<small>$line</small>" ;;
		*) text='<span foreground=\"#223249\">⏹</span>' ;;
		esac
		...
		#
    # '{\"text\": \"{{markup_escape(title)}}\", \"tooltip\": \"{{playerName}} : {{markup_escape(title)}}\", \"alt\": \"{{status}}\", \"class\": \"{{status}}\"}' -F",
	...
	# no current players
	# exit if print fails
	echo '<span foreground=#C34043>⏹</span>' || break
	sleep 15
